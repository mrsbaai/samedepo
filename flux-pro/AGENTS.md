...
6. **Get visual parity on the core states before broadening the API.** For trigger/popover-style components, verify these first on the docs page: base trigger, open popover, disabled state, invalid state, and size variants. Don't move on to extra props until these look right.
7. **Audit outside-in** -- browse the docs page with Playwright, stress test interactions, inspect markup, then audit JS.
8. **Verify everything with Playwright** before claiming done.
...
- **Copy sibling structure before styling.** If a trigger is conceptually an input, copy the sibling input pattern first and only then adapt it. Don't invent a wrapper-heavy structure and hope to style it into parity later.
...
- **Critical visual states need explicit checks.** Empty states, white-on-white states, disabled states, and focus-visible states routinely hide problems. Inspect those on the real docs page before considering the styling done.
...
- **On new components, verify the same small set of states repeatedly:** base, open, disabled, invalid, sizes, keyboard focus, and empty state. These catch most regressions early.
