<div class="py-8">
    <div class="mb-6">
        ...
        <flux:subheading class="mt-2">Every deposit, withdrawal, and ledger adjustment across all owners.</flux:subheading>
    </div>
                ...
                @foreach (range(1, 5) as $r)
                    <flux:table.row>
                        @foreach (range(1, 9) as $c)
                            <flux:table.cell @class(['max-md:hidden' => in_array($c, [4, 5, 7, 9])])><flux:skeleton class="h-4 w-16" /></flux:table.cell>
                        @endforeach
                    </flux:table.row>
                @endforeach
            </flux:table.rows>
    ...
    @else
        <div class="flex flex-wrap items-center gap-3 mb-6">
            <flux:input icon="magnifying-glass" wire:model.live.debounce.300ms="search" placeholder="Search owner, reference, or tx hash..." size="sm" class="w-full sm:max-w-xs" clearable />
        ...
        @else
            ...
            <flux:table :paginate="$this->paginatedEntries" pagination:scroll-to>
                <flux:table.columns>
                    <flux:table.column>Time</flux:table.column>
                    <flux:table.column class="max-lg:hidden">Owner</flux:table.column>
                    <flux:table.column>Type</flux:table.column>
                    <flux:table.column class="max-md:hidden">Reference</flux:table.column>
                    <flux:table.column class="max-md:hidden">Network</flux:table.column>
                    <flux:table.column>Gross</flux:table.column>
                    ...
                    <flux:table.column>Net</flux:table.column>
                    <flux:table.column class="max-md:hidden">Status</flux:table.column>
                    <flux:table.column align="end">Tx</flux:table.column>
                </flux:table.columns>
                <flux:table.rows>
                    @foreach ($this->paginatedEntries as $tx)
                        <flux:table.row wire:key="tx-{{ $tx['id'] }}">
                            ...
                            <flux:table.cell class="max-lg:hidden max-w-48 truncate">
                                @if ($tx['ownerId'])
                                    <flux:link href="{{ route('admin.owners.show', $tx['ownerId']) }}" wire:navigate>{{ $tx['ownerEmail'] ?? 'Owner #'.$tx['ownerId'] }}</flux:link>
                                @else
                                    &mdash;
                                @endif
                            </flux:table.cell>
                            ...
                            <flux:table.cell class="max-md:hidden">
                                @if ($tx['customer'] && $tx['ownerId'])
                                    <flux:link href="{{ route('admin.owners.customers.show', [$tx['ownerId'], $tx['userRef']]) }}" wire:navigate>{{ $tx['userRef'] }}</flux:link>
                                @else
                                    {{ $tx['userRef'] ?? '—' }}
                                @endif
                            </flux:table.cell>
