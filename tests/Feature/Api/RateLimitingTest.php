...
test('requests within quota return existing status and rate limit headers', function () {
    ...
    $response = $this->withHeaders($headers)
        ->getJson('/api/v1/customers/CUST-001')
        ->assertCreated();
...
test('the first request over quota returns json 429 with retry after and rate limit headers', function () {
    ...
    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-001')->assertCreated();
    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-002')->assertCreated();

    $response = $this->withHeaders($headers)
        ->getJson('/api/v1/customers/CUST-003')
        ->assertStatus(429)
...
test('rate limit counters are isolated per api key', function () {
    ...
    $this->withHeaders(rateLimitHeaderForKey($keyA, 'sm_api_ratelimit_testkey_a'))
        ->getJson('/api/v1/customers/CUST-A1')
        ->assertCreated();
    $this->withHeaders(rateLimitHeaderForKey($keyA, 'sm_api_ratelimit_testkey_a'))
        ->getJson('/api/v1/customers/CUST-A2')
        ->assertCreated();

    $this->withHeaders(rateLimitHeaderForKey($keyA, 'sm_api_ratelimit_testkey_a'))
        ->getJson('/api/v1/customers/CUST-A3')
        ->assertStatus(429);

    $this->withHeaders(rateLimitHeaderForKey($keyB, 'sm_api_ratelimit_testkey_b'))
        ->getJson('/api/v1/customers/CUST-B1')
        ->assertCreated();
});
...
test('quota resets after the one minute window', function () {
    ...
    $this->withHeaders($headers)
        ->getJson('/api/v1/customers/CUST-002')
        ->assertStatus(429);
...
test('invalid or revoked credentials do not consume quota or trigger 429', function () {
    ...
    $this->getJson('/api/v1/customers/CUST-001')
        ->assertUnauthorized()
...
test('updated admin setting is used for subsequent requests without cache changes', function () {
    ...
    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-001')->assertCreated();
    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-002')->assertCreated();
    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-003')->assertStatus(429);

    PlatformSettings::instance()->update(['api_requests_per_minute' => 4]);

    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-004')->assertCreated();
    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-005')->assertCreated();
    $this->withHeaders($headers)->getJson('/api/v1/customers/CUST-006')->assertStatus(429);
});
